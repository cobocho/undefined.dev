...
function generatePostSitemap(post: Omit<Post, "content">) {
  const images = Object.values(post.images).map((image) => {
    return `<image:image><image:loc>${SITE_URL}${image.src}</image:loc></image:image>`;
  });

  return `<url>
    <loc>${SITE_URL}/post/${post.category}/${post.slug}</loc>
